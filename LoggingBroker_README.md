...
```bash
git clone https://github.com/emreinan/loggingbroker.git
```

2. Start the service using Docker Compose:
```bash
cd loggingbroker/src/logging-service/logging-broker
docker-compose up -d
```

3. To test using Postman:
- Import the file `docs/LoggingBroker.postman_collection.json` into Postman.
...
Query logs with filters. Full usage is documented in `docs/LoggingBroker-API-Docs.md`.
...
- `LoggingBroker.postman_collection.json` → Postman requests
- `LoggingBroker-API-Docs.md` → Full API reference
